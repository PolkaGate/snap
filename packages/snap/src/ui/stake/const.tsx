import type { HexString } from "@polkadot/util/types";
import { PASEO_ASSET_HUB_GENESIS_HASH, STATEMINE_GENESIS_HASH, STATEMINT_GENESIS_HASH, WESTMINT_GENESIS_HASH } from "../../constants";

export const POLKADOT_GENESIS_HASH = '0x91b171bb158e2d3848fa23a9f1c25182fb8e20313b2c1eb49219da7a70ce90c3';
export const KUSAMA_GENESIS_HASH = '0xb0a8d493285c2df73290dfb7e61f870f17b41801197a149ca93654499ea3dafe';
export const WESTEND_GENESIS_HASH = '0xe143f23803ac50e8f6f8e62695d1ce9e4e1d68aa36c1cd2cfd15340213f3423e';
export const PASEO_GENESIS_HASH = '0x77afd6190f1554ad45fd0d31aee62aacc33c6db0ea801129acb813f913e0764f';

export const POLKAGATE_POOL_IDS: Record<string, number> = {
  [POLKADOT_GENESIS_HASH]: 8, // can be removed after migration
  [STATEMINT_GENESIS_HASH]: 8,
  [KUSAMA_GENESIS_HASH]: 18, // can be removed after migration
  [STATEMINE_GENESIS_HASH]: 18,
  [WESTMINT_GENESIS_HASH]: 6,
  [PASEO_ASSET_HUB_GENESIS_HASH]: 3,
};

export const STAKING_TEST_CHAINS = [
  WESTMINT_GENESIS_HASH,
  PASEO_ASSET_HUB_GENESIS_HASH
] as HexString[];

export const STAKING_CHAINS = [
  POLKADOT_GENESIS_HASH,
  KUSAMA_GENESIS_HASH,
  ...STAKING_TEST_CHAINS
] as HexString[];

export const REWARDS_SAVED_INFO_VALIDITY_PERIOD = 15 * 60 * 1000;
export const DEFAULT_DECIMAL_POINT = 4;
export const MAX_POOL_NAME_TO_SHOW = 25;
